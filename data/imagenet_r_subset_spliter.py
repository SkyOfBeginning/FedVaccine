...
from data.continual_datasets import Imagenet_R
...
class ImagenetR_spliter():
    ...
    def random_split(self):
        ...
        self.Imagenet_R = Imagenet_R(root='D:/datasets/local_datasets', train=True, download=True)
    ...
    def random_split_synchron(self):
        trans = build_transform(False,self.input_size)
        self.Imagenet_R = Imagenet_R(root='/tmp/pycharm_project_26/D:/datasets/local_datasets', train=True, download=True)
        trainset = self.Imagenet_R
    ...
    def random_split_DLG(self):
        trans = build_transform(True,self.input_size)
        self.cifar100_dataset =Imagenet_R(root='D:/datasets/local_datasets', train=True, download=True)
    ...
    def get_test_data(self):
        self.Imagenet_R_test = Imagenet_R(root='D:/datasets/local_datasets', train=False, download=True)
        testset = self.Imagenet_R_test
...
class CustomedSubset(Dataset[T_co]):
    ...
    def __init__(self, dataset: Dataset[T_co], indices: Sequence[int], trans, show_sample) -> None:
        ...
        self.is_buffer =np.zeros(len(self.targets))

    def __getitem__(self, idx):
        img, target = self.data[idx], self.targets[idx]
        is_buffered = self.is_buffer[idx]  # Get the buffer flag for this sample
    ...
    def expand(self, newdataset: Dataset[T_co]):
        new_data_list = []
        new_targets_list = []
        # Create a list for the new is_buffer flags (all 1s)
        new_is_buffer_flags = []

        for i in len(newdataset):
            new_data_list.append(newdataset.data[i])
            new_targets_list.append(newdataset.targets[i])
            new_is_buffer_flags.append(1)  # Mark as buffered/appended data

        # Efficiently extend existing lists and numpy array
        self.data.extend(new_data_list)
        self.targets = np.concatenate((self.targets, np.array(new_targets_list)))
        self.is_buffer = np.concatenate((self.is_buffer, np.array(new_is_buffer_flags, dtype=int)))
...
class Test_set(Dataset[T_co]):
    def __init__(self, dataset: Dataset[T_co]):
        self.dataset = dataset
        self.data = self.dataset.data
        self.targets  = self.dataset.targets
        self.trans = build_transform(False,224)
    ...
    def get_data(self,target_class):
        # 1. 查找对应类别的索引
        # 使用布尔索引：self.targets == target_class 会返回一个布尔数组
        class_mask = np.isin(self.targets, target_class)

        # 2. 找到所有为 True 的索引位置
        class_idx = np.where(class_mask)[0]

        # 3. 提取对应的图像数据
        # 如果 self.data 是列表，使用列表推导式
        class_data = [self.data[i] for i in class_idx]

        # 如果 self.data 是 NumPy 数组或 Tensor，可以直接用 mask 索引：
        # class_data = self.data[class_mask]

        # 4. 提取对应的标签
        class_targets = [self.targets[i] for i in class_idx]

        # 3. 返回结果
        return subsubset(class_data, class_targets, self.trans)
