...
class Attention(nn.Module):
    ...
    def forward(self, x, A=None, Q=None, scale=None):
        B, N, C = x.shape

        qkv = self.qkv(x)

        if A is not None:
            x_proj = self.attn_drop(x)  # (batch, 1, in_features)
            # print('666这人开挂')
            # print(qkv.shape,x_proj.shape,A.shape,Q.shape)
            lora_out = (x_proj @ A.T) @ Q.T
            qkv = qkv + scale * lora_out  # qkv新输出
...
class Block(nn.Module):
    ...
    def forward(self, x, A=None, Q=None, scale=None):
        x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x), A, Q, scale)))
...
class VisionTransformer(nn.Module):
    ...
    def forward_features_with_LoRA(self,x,A_s,B_s,index,scaling):
        x = self.patch_embed(x)  # 把图片切成patch
        ...
        for i, block in enumerate(self.blocks):
            if i in index:
                positions = index.index(i)
                # x = block(x, A=A_s[positions], Q=B_s[positions], scale=scaling)
                x = block(x, A=A_s, Q=B_s, scale=scaling)
    ...
    def forward_features_with_prompt(self, x, prompts):
    ...
    def forward(self, x):
        ...
        # res = self.forward_features_with_LoRA(x,A_s,B_s,index,scaling)
