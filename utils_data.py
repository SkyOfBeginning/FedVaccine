...
def get_real_grad( x_real: Tensor, y_real: Tensor, model: nn.Module, fc: nn.Module) -> Tensor:
    ...
    real_augmentor = get_augmentor(aug_mode="standard", crop_res=224)
    with torch.amp.autocast('cuda', enabled=True):
        ...
        # (x - mean) / std
        x_real = (x_real - torch.tensor(mean, device="cuda").reshape(1, 3, 1, 1)) / torch.tensor(std, device="cuda").reshape(1, 3, 1, 1)
...
def get_syn_grad(x_syn: Tensor, y_syn: Tensor, model: nn.Module, fc: nn.Module) -> Tensor:

    # auto-casting was a *huge* speed up
    mean = [0.485, 0.456, 0.406]
    std = [0.229, 0.224, 0.225]
    syn_augmentor = get_augmentor(aug_mode="standard", crop_res=224)
    with torch.amp.autocast('cuda', enabled=True):
